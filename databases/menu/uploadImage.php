...
if(isset($_GET["remove"])){
    $stmt = $pdo->prepare("UPDATE menu SET image=null WHERE id=:id");
    $stmt->execute([':id' => $_GET["id"]]);
    echo json_encode(["status" => "success", "message" => "You removed image of this menu already."]);
    exit;
}else{
    if($_FILES["fileToUpload"]["error"] == 0){
        ...
        if ($uploadOk == 1) {
            ...
            if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file_new)){
                ...
                echo json_encode(["status" => "success", "message" => "You upload new menu picture already."]);
                exit;
            }else{
